...
The supported input adapter is the multi-sheet Excel workbook generated by `runscripts/create_well_input_workbook.py` or the ready-to-edit Wildcat and Smeaheia examples. It contains physical well data (`Header`, `Survey`, `HoleCasings`, `Plugs`, and `Stratigraphy`) plus editable GaP/CIRRUS `GridPolicy` and `SubsurfaceAssumptions` sheets. SCREEN parses only the physical sheets into canonical WellClass JSON; simulation assumptions are parsed separately into a GaP `SimulationDesign`. Leave `Survey` empty for a vertical well; enter measured depth, inclination, and azimuth rows for a deviated survey.

The CSV/YAML examples below are retained for compatibility and historical reference. New workflows should prefer canonical JSON or the workbook adapter.
