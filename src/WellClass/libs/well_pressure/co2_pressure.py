...
def _integrate_pressure(
    well_header: dict, pt_df_in: pd.DataFrame, get_rho: callable, reference_depth: float, reference_pressure: float, direction: str, colname_p: str
) -> pd.DataFrame:
    """
    Simple integration to find pressure
    Starting point: reference_depth at reference pressure.  Reference temperature is found in pt_df
    ...
    # Integrate pressure
    solution = solve_ivp(
        _Pdz_odesys, [z0, z_final], [p0], args=(well_header, compute_T, get_rho), t_eval=query["depth_msl"].values, dense_output=True, method="Radau"
    )
