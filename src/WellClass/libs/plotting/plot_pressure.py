...
def plot_pressure(
    ...
    *,
    ...
    """
    pressure vs depth
    Takes the Pressure class, geology, and physical plug intervals.

    ``barriers`` remains an alias for older callers and should be replaced by
    ``plugs`` in new code.
    ...
    # Draw cement plugs
    for idx, row in plugs_df.iterrows():
    ...
    for idx, p_sc in scenarios.iterrows():
        ...
        # plot water gradient
        pt_df[pt_df[("init", "depth_msl")] >= sf_depth].plot(
            x=(sc_name, "h2o"), y=("init", "depth_msl"), ax=ax, label="_nolegend_", color="steelblue", legend=False, lw=0.75, ls=ls_list[counter]
        )

        # plot CO2 gradient
        pt_df[pt_df[("init", "depth_msl")] >= sf_depth].plot(
            x=(sc_name, "co2"), y=("init", "depth_msl"), ax=ax, label=sc_label, color="firebrick", legend=False, lw=0.75, ls=ls_list[counter]
        )
        counter += 1
