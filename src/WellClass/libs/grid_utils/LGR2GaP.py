...
def df_to_gap_casing(holes_df: pd.DataFrame, casings_df: pd.DataFrame, LGR_NAME: str, O: TextIO) -> None:  # noqa: E741
    ...
    for idx in casings_df.index:
        ...
        for ic, (_, row) in enumerate(df_holes_new.iterrows()):
            ...
            CARFIN_cement_bond(
                ...
                int(ij_min_pipe + 1),
                int(ij_max_pipe + 1),
                int(new_toc_k_min_cb + 1),
                int(new_toc_k_max_cb + 1),
                x_bd=x_thickness,
                y_bd=x_thickness,
                perm=cb_perm,
                LGR_NAME=LGR_NAME,
