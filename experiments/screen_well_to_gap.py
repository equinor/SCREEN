...
plot_pressure(my_pressure, my_well.geology, my_well.barriers, ax=ax2, plot_HSP=True, plot_RP=True, plot_MSAD=True, plot_maxP=True)
fig.tight_layout()
...
equil_pmts = {
    "overburden_water": {"DATUM_D": overburden_datum_d, "PRESSURE": overburden_pressure, "WGC_D": None},
    "CO2_column": {"DATUM_D": reservoir_datum_d, "PRESSURE": reservoir_pressure, "WGC_D": reservoir_wgc_d},
}

new_lines = lines[:236]
for name, pmts in equil_pmts.items():
    equil_lines = setup_equilibration(
        name=name,
        pressure=pmts["PRESSURE"],
        d_datum=pmts["DATUM_D"],
        wgc_d=pmts["WGC_D"],
        rtempvd=new_rtempvd_section,
        top=z_top_RTEMPVD,
        bottom=z_bottom_RTEMPVD,
    )
...
gap_casing_df = lgr.build_grdecl(output_include_dir, LGR_NAME, drilling_df, casings_df, barrier_regions_df)

# Update EQLNUM
top_eql_k = barrier_regions_df.iloc[0]["k_max"] + 1
