...
- Design a first-class simulation-scenario layer for one physical wellbore. A workbook must be able to tabulate multiple named scenarios and generate a separate reproducible case directory, parameterized deck/GRDECL, LGR output, logs, and results for each scenario. Scenario fields should include pressure/contact assumptions, grid policy, open-hole/cement-bond/plug defaults and per-item overrides, reservoir/aquifer permeability, salinity, and explicit defect variants. Define precedence as: item-specific well override -> selected scenario override -> documented default.
